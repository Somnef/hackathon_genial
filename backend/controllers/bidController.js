...
const createBid = async (req, res, next) => {
    ...
    try {
        ...
        const receipt = await contract.methods
            .placeBid(offerId, web3.utils.toWei(bidAmount.toString(), 'ether')) // Convert to wei
            .send({ from: bidder });
...
// Controller to list all bids for an offer (optional)
const listBids = async (req, res, next) => {
    const { offerId } = req.params; // Get the offerId from the URL
